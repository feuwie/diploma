...
public class jwtUtil {
	...
	private final static String SESUUID = "SESSION_UUID";

	@Autowired
	private UserRepository userRepo;

	public String createToken(String session_phone, String session_type) {
	...
	public String createAnonToken(UUID session_uuid) {
		Map<String, Object> map = new HashMap<>();
		map.put(SESUUID, session_uuid);
	...
	// public String createMailToken(String session_email, String session_pass,
	// String session_type) {
	// Map<String, Object> map = new HashMap<>();
	...
	public User checkToken(String token) {
		...
		// User user = userRepo.findByEmailAndUsertype(claim.get(SES_EMAIL).toString(),
		// claim.get(SES_TYPE).toString());
		User user = userRepo.findByPhoneAndUsertype(claim.get(SES_PHONE).toString(), claim.get(SES_TYPE).toString());
		return user;
	}
